...
const filename = "YTD Sales Report - JonS.xls";
...
for (let i = 0; i < dimensions[4]; i++) {
  ...
  columnsArray.forEach((column) => {
    ...
    if (cell && cell.v && !String(cell.v).startsWith("Page ")) {
      const dbCol = headers[alphaToNum(column)].replace(/[^0-9a-zA-Z$_]/g, "");
      cells[dbCol] = cell.v;
    }
  });
...
pool.getConnection((err, connection) => {
  ...
  for (const record of records) {
    let sql = "INSERT INTO Orders2 SET ?";
    let values = record;
    try {
      // Use the connection
      //console.log(values);
